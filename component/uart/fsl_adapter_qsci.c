...
static const IRQn_Type s_QsciTxIdleIRQ[] = QSCI_TRIDLE_IRQS;
#elif defined(QSCI_TIDLE_IRQS)
static const IRQn_Type s_QsciTxIdleIRQ[] = QSCI_TIDLE_IRQS;
#endif
...
static void HAL_UartInterruptHandle(uint8_t instance)
{
    ...
    if ((kQSCI_RxDataRegFullFlag & status) &&
        (QSCI_GetEnabledInterrupts(s_QsciAdapterBase[instance]) & kQSCI_RxFullInterruptEnable))
    {
        clearStatus |= kQSCI_RxDataRegFullFlag | kQSCI_Group1Flags;
        if (uartHandle->rx.buffer)
        {
            ...
            {
                QSCI_DisableInterrupts(s_QsciAdapterBase[instance],
    ...
    if ((kQSCI_TxDataRegEmptyFlag & status) &&
        (QSCI_GetEnabledInterrupts(s_QsciAdapterBase[instance]) & kQSCI_TxEmptyInterruptEnable))
    {
        clearStatus |= kQSCI_TxDataRegEmptyFlag | kQSCI_TxIdleFlag;
        if (uartHandle->tx.buffer)
        {
            ...
            {
                QSCI_DisableInterrupts(s_QsciAdapterBase[uartHandle->instance], kQSCI_TxEmptyInterruptEnable);
...
#if 1
    QSCI_ClearStatusFlags(s_QsciAdapterBase[instance], clearStatus);
#endif
}
#endif
...
hal_uart_status_t HAL_UartInit(hal_uart_handle_t handle, const hal_uart_config_t *config)
{
    ...
    assert(handle);
    assert(config);
    assert(config->instance < (sizeof(s_QsciAdapterBase) / sizeof(QSCI_Type *)));
    assert(s_QsciAdapterBase[config->instance]);
    assert(HAL_UART_HANDLE_SIZE >= sizeof(hal_uart_state_t));

    QSCI_GetDefaultConfig(&qsciConfig, config->baudRate_Bps, config->srcClock_Hz);
    ...
    qsciConfig.bEnableRx = (bool)config->enableRx;
    qsciConfig.bEnableTx = (bool)config->enableTx;

    status = QSCI_Init(s_QsciAdapterBase[config->instance], &qsciConfig);
...
#if (defined(HAL_UART_TRANSFER_MODE) && (HAL_UART_TRANSFER_MODE > 0U))
    QSCI_TransferCreateHandle(s_QsciAdapterBase[config->instance], &uartHandle->hardwareHandle,
                              (qsci_transfer_callback_t)HAL_UartCallback, handle);
#else
    s_UartState[uartHandle->instance] = uartHandle;
    /* Enable interrupt in */

    EnableIRQWithPriority(s_QsciRxIRQ[config->instance], HAL_UART_ISR_PRIORITY);
    EnableIRQWithPriority(s_QsciTxIRQ[config->instance], HAL_UART_ISR_PRIORITY);
    EnableIRQWithPriority(s_QsciRxErrIRQ[config->instance], HAL_UART_ISR_PRIORITY);
#endif
...
hal_uart_status_t HAL_UartReceiveBlocking(hal_uart_handle_t handle, uint8_t *data, size_t length)
{
    ...
    status = QSCI_ReadBlocking(s_QsciAdapterBase[uartHandle->instance], data, length);
...
hal_uart_status_t HAL_UartSendBlocking(hal_uart_handle_t handle, const uint8_t *data, size_t length)
{
    ...
    QSCI_WriteBlocking(s_QsciAdapterBase[uartHandle->instance], data, length);
...
hal_uart_status_t HAL_UartSendNonBlocking(hal_uart_handle_t handle, uint8_t *data, size_t length)
{
    ...
    QSCI_EnableInterrupts(s_QsciAdapterBase[uartHandle->instance], kQSCI_TxEmptyInterruptEnable);
    return kStatus_HAL_UartSuccess;
}
...
hal_uart_status_t HAL_UartAbortReceive(hal_uart_handle_t handle)
{
    ...
    if (uartHandle->rx.buffer)
    {
        QSCI_DisableInterrupts(s_QsciAdapterBase[uartHandle->instance],
                               kQSCI_RxDataRegFullFlag | kQSCI_RxErrorInterruptEnable);
        uartHandle->rx.buffer = NULL;
    }
