...
FLASH_PARTITION {
  ...
  # FlashPart1 : ORIGIN_P = 0x0,     ORIGIN_X = 0x20000, LENGTH = 0x8000
  # FlashPart2 : ORIGIN_P = 0x8000, ORIGIN_X = 0x28000, LENGTH = 0x8000
}
...
MEMORY {
  ...
  .p_RamCode     (RX): ORIGIN = 0x60000, LENGTH = 0x6000           # RAM mapped into p space, code resides in flash and runs in RAM

  .p_RamData     (RX): ORIGIN = AFTER(.p_RamCode) - 0x60000, LENGTH = 0x6000 + 0x60000 - AFTER(.p_RamCode)  # .data section
  .x_Data        (RW): ORIGIN = AFTER(.p_RamCode) - 0x60000, LENGTH = 0x6000 + 0x60000 - AFTER(.p_RamCode)  # bss, heap, stack ...
